...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def total(c: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = c match {
      case Nil => acc
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.sortBy(node => node._2).map((n) => Leaf(n._1, n._2))
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case Nil => trees
    case x :: Nil => trees
    case left :: right :: xs => (makeCodeTree(left, right) :: trees.tail.tail).sortBy(node => weight(node))
  }
  ...
  /**
    ...
    */
  def until(isSingleton: List[CodeTree] => Boolean, combineFunc: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree = {
    if (isSingleton(trees)) trees.head
    else (until(isSingleton, combineFunc)(combineFunc(trees)))
  }
